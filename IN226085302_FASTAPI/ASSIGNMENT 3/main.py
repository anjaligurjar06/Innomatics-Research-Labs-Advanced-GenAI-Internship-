...
def audit_products():
    in_stoct_list = [p for p in products if p['in_stock']]
    out_stoct_list = [p for p in products if not p['in_stock']]
    stock_value = sum(p['price'] * 10 for p in in_stoct_list)  # Assume 10 units each for value calc
    expensive = max(products, key=lambda p: p['price'])
    return {
        'total_products': len(products), 'in_stock': len(in_stoct_list), 'out_of_stock': [p['name'] for p in out_stoct_list],
        'total_stock_value': stock_value, 'most_expensive': {'name': expensive['name'], 'price': expensive['price']}
    }

# ----------- Bonus Question 6: Apply a Category-Wide Discount ------------
@app.put('/products/discount')
def apply_discount(
    category: str = Query(..., description='Category to discount'),
    percent:  int  = Query(..., ge=1, le=99, description='% off'),
):
    updated_products = []
    for p in products:
        if p['category'] == category:
            p['price'] = int(p['price'] * (100 - percent) / 100)
            updated_products.append(p)
    if not updated_products:
        return {'message': f'No products found in category "{category}"'}
    return {'message': f'{percent}% discount applied to {category}', 'updated_products': len(updated_products), 'products': updated_products}
